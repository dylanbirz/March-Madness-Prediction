...
model_data <- read.csv("/Users/dylanbirzon/Desktop/capstone/model_data.csv",header=TRUE)
#View(model_data)
...
model <- glm(winner ~ poss_A + rtg_A + poss_B + rtg_B, family = binomial(link = "logit"), data = train_data)
summary(model)

#prediction on validation set
pred_validation <- predict(model,newdata = validation_data, type="response")

#combine validation with predicted probabilities
validation_data$predicted_winner <- ifelse(pred_validation > 0.5, 1, 0)

#compare predicted winners to actual winners
validation_data$prediction_correct <- ifelse(validation_data$predicted_winner==validation_data$winner,"Correct","Incorrect")
